...
3. **Board** — added 2026-07-01, iterated several times same day/next on Chris's direct feedback. Pick one Class + Position group (RHP/LHP/INF/OF/C, via `sheet_write.pos_group()`) at a time; shows tier 0.1/1/2 only, same as the sheet's own `★ < 3` filter. Each row is a real `st.popover` trigger button (emoji tier badge, e.g. "🔵 C", now its own leftmost column - not fused into the name) - clicking it opens an inline panel with that player's notes, tier pills, note field, and save button.
   ...
   - **2026-07-02, eighth pass: header was rendering at ~9.6px tall, cut off.** Root cause found via computed-style inspection, not guessed: `overflow-x: auto` was applied to the *same* element that also held the header's (short, button-less) content, and Streamlit's own layout wrapper resolved height against that overflow box instead of the content's natural height. Fix: horizontal scrolling now lives on exactly one outer wrapper around the whole header+rows stack (`st.container(key="nb_board_rows")`, `overflow-x: auto` only there) so header and rows scroll together in sync, and every row/header gets an explicit `min-height` instead of trusting short content to size it correctly.
     - Also this pass: columns other than Name/Team shrunk (26-34px instead of 40-50px - short 1-3 char values at 11px font don't need that much room, "less blank space means more content"); vertical gap between player rows dropped from a default ~16px to ~2px (found via live DOM inspection that the *outermost* `.st-key-nb_board_rows` element is itself a `stVerticalBlock` with the 16px gap - a descendant-only CSS selector missed it since it doesn't match the element itself, only its children); and the header row is now `position: sticky; top: 0` in its own `st.container(key="nb_board_header")`, so it stays visible while scrolling through the player list ("freeze label row as we scroll") - verified live by scrolling the actual page content past several rows and confirming the header stayed pinned at the top.
