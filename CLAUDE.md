...
3. **Board** — added 2026-07-01, iterated several times same day/next on Chris's direct feedback. Pick one Class + Position group (RHP/LHP/INF/OF/C, via `sheet_write.pos_group()`) at a time; shows tier 0.1/1/2 only, same as the sheet's own `★ < 3` filter. Each row is a real `st.popover` trigger button (emoji tier badge, e.g. "🔵 C", now its own leftmost column - not fused into the name) - clicking it opens an inline panel with that player's notes, tier pills, note field, and save button.
   ...
   - **Known trade-off, not yet resolved:** the earlier static-HTML version let Team render at a visibly smaller font than the rest of the row (Chris asked for this explicitly). `st.dataframe` is canvas-rendered (glide-data-grid) - column-level font-size isn't exposed the way per-cell HTML/CSS was, so Team is currently the same size as every other column. Flagged, not silently dropped - revisit if Chris asks for it specifically.
   - **Lesson, reinforced a third time this session:** verify Streamlit widget/API behavior live before building on an assumption, even for "should be simple" things like column layout. `st.columns()` stacking on mobile and `width="stretch"`'s actual semantics were both discovered by testing at the real target viewport (390px), not by reading the return type hints.
